...
def read_boxes_from_csv(csv_file, conf_threshold=0.5):
    boxes = {}
    img = []
    with open(csv_file, 'rt') as f:
        ...
        for i,row in enumerate(reader):
            if i > 0:
                ...
                row[6] = float(row[6])
                if row[6] < conf_threshold: continue
...
def eval_detector_csv(gt_csv_file, detector_csv_file, iou_threshold, conf_threshold):
    ground_truth_boxes_by_img = read_boxes_from_csv(gt_csv_file, conf_threshold)
    if DEBUG:
        print ('Ground truth: cvs - {} , Number images = {}'.format(gt_csv_file,len(ground_truth_boxes_by_img)))

    detector_boxes_by_img = read_boxes_from_csv(detector_csv_file, conf_threshold)
    if DEBUG:
...
def main():
    ...
    iou_threshold = 0.75
    print ('{}'.format(eval_detector_csv(gt_csv, dt_csv, iou_threshold, float(args.conf))))
